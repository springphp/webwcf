...
class Index extends AdminBase{
    ...
    public function main(){
        $nend_check_count = model('member')->where(['is_check'=>0])->count();
        $user_count = model('member')->count();
        //待催收统计
        $collection_count = model('collection')->where(['status'=>1])->count();
        //投资统计
        $invest_moeny = model('order')->where(['is_done'=>1])->column('money');
        $total_invest  = array_sum($invest_moeny);
        $invest_count = count($invest_moeny);
        //借款统计
        $borrow_moeny = model('order')->column('money');
        $total_borrow  = array_sum($borrow_moeny);
        $borrow_count = count($borrow_moeny);
        $this->get_dataImg();
        return view('',['nend_check_count'=>$nend_check_count,'user_count'=>$user_count,'total_invest'=>$total_invest,
            'total_borrow'=>$total_borrow,'invest_count'=>$invest_count,'borrow_count'=>$borrow_count,'collection_count'=>$collection_count,'date'=>date('Y-m-d',time())]);
    }
